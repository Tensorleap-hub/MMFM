...
import argparse
from mmfm.gcs_utils import _download, _connect_to_gcs_and_return_bucket
from mmfm.config import cnf

class InstructData:
    def __init__(self, args):
        base_dir = Path(__file__).resolve().parent
        self.instruction_path = base_dir / "metadata_files" / Path('instructdoc_instructions.xlsx')
        ...
        self.gcs_bucket_name = args.gcs_bucket_name
        self.gcs_data_dir = args.gcs_data_dir
        self.processed_data_dir = args.processed_data_dir


    def create_data(self):
        bucket = _connect_to_gcs_and_return_bucket(self.gcs_bucket_name)
        test_path_1 = os.path.join(self.processed_data_dir, f'{self.out_data_dir}_fill_in_blank', f'converted_output_test.json')
        # test_path_1 = _download(bucket_name=self.gcs_bucket_name, cloud_file_path=test_path_1)
        with open(test_path_1, "r") as f:
            data = json.load(f)
        ids_list = [item["id"] for item in data]

        test_path_2 = os.path.join(self.processed_data_dir, f'{self.out_data_dir}_choose_txt', f'converted_output_test.json')
        # test_path_2 = _download(bucket_name=self.gcs_bucket_name, cloud_file_path=test_path_2)
        ...
        for i, split in enumerate(self.split):
            ...
            for answer_style in ['fill_in_blank', 'choose_txt']:
                ...
                for j, file_path in enumerate(blobs):       # HERE
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--gcs_data_dir', type=str, help="GCS data directory prefix")
    #
    #
    # args = parser.parse_args()

    dataset = InstructData(cnf)
    dataset.create_data()