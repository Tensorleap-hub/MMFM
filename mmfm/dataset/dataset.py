...
class IconQAFeatureDataset(Dataset):
    ...
    def tensorize(self):
        for entry in self.entries:
            ...
            c_token = np.zeros((self.c_num, self.max_choice_length))
            choice_num = len(entry['c_token'])
            c_token[:choice_num, :] = np.array(entry['c_token'])
            entry['c_token'] = c_token

    def get_image_path(self, index):
        return f'iconqa/{self.name}/{self.task}/{index}/image.png'
